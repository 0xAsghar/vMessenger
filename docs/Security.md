...
| L6 | **Operator key is a placeholder** | `NetworkConfig.OPERATOR_ED25519_PUBLIC_KEY_HEX` is 64 zeros, so `operatorEd25519PublicKey()` returns null and **no** `SignedNodeRecord` can ever be `OFFICIAL`. It is still unset in 2.0.2; until it is set in a release, the operator-trust tier is dead code. |
...
| L19 | **Audit retention reverses local erasure for the groups that enable it** | Off by default and creator-only, scoped to one group, never applied to a 1:1 chat, capped at 90 days and readable only on the device that captured it (§14). Switching it on or off is written as a system line into every member's copy of the group's history, and a member who joins while it is on gets the line too (since 2.0.2; before, only the creator's copy had it). There is no standing banner; an admin also sees the review entry (§14). Within those bounds it is still what it looks like: a member of such a group is trusting that group's admins with the text of what they edited or withdrew. Leaving is the only opt-out. |
...
| **Announced** | a system line in every member's copy of the group's history when the policy is switched on or off: the creator's device writes it in `GroupRepositoryImpl.setAuditRetention`, and each member's device when the `SNAPSHOT` changes the policy, or when it joins a group that has it on (`GroupControlHandler.applySnapshot`, since 2.0.2). The group screen carries no standing banner and only the creator sees the switch; while it is on, an admin also sees the review entry on the group screen. The app runs on organization-managed phones whose staff are told how it works when it is installed |
...
| **Switching it off erases — on every device** | `GroupAuditHistory.erase` deletes the group's captures and the attachment files only they kept, on the creator's device when the creator switches it off and on each member's when the snapshot arrives (since 2.0.2; before, members' devices kept theirs). Leaving a stockpile behind would mean members are still exposed by a rule that no longer applies. A device that leaves or is removed erases too, since it no longer hears the switch; and at each start `GroupAuditHistory.eraseLeftovers` erases captures of any group whose review is off or that the device no longer holds — what a member's 2.0.1 device kept. A snapshot naming a creator other than the one stored for the group is dropped, so a member cannot erase another device's captures by claiming the group |
| **Not in a backup** | the backup payload carries the identity, contacts, location grants, user-added nodes and optionally one-to-one conversations with their messages (not groups, not messages on a timer); this table is not in it, so retained content does not travel off the device that recorded it |

Only messages that were **actually sent** and later edited or withdrawn are captured — the text as it stood, the caption, the attachment name and path, the author's identity hash, and when this device saw the revision. A draft is never captured. With retention on, the attachment file is kept so a captured row still resolves; with it off the file goes as it always has. A file kept for a deleted message is referenced only by its capture. Switching retention off erases it with the capture; the 90-day purge and the cascade from its message or conversation do not, and it then stays, encrypted and unreferenced, in `files/attachments` until a wipe.
