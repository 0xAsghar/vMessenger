...
| [`BuiltInBootstrapProvider`](../network/bootstrap/src/main/kotlin/ir/vmessenger/network/bootstrap/BuiltInBootstrapProvider.kt) | 100 | The shipped default, `NetworkConfig.effectiveBootstrapAddress()`, only in legacy mode (`P2PConfig.multiNodeEnabled` off) or with the emulator dev bootstrap. With multi-node on (the default) it contributes nothing: the enabled `bootstrap_node` rows are the only source, so switching the built-in row off takes the built-in node out. |
...
At first run the app asks which node to use: the built-in test nodes (which the app says will be removed on 31 December 2026), an address of one's own, a node set up with New node, or none for now. Only "none" stops the built-in rows being seeded into `bootstrap_node` and `relay_node` (`DiscoveryRepositoryImpl.joinNetwork`). Since 2.0.2 nothing brings the built-in node back as a fallback: with no bootstrap row enabled the device is not in the DHT, and with no relay row enabled it listens on no relay and publishes only a direct endpoint, if it has one (`RelayDirectoryImpl.activeRelay` returns none, `RelayListener.noRelay`). Before, the built-in node stayed in use whatever the switches said. A switch takes effect at once (`RelayControl.reselectRelay`): the relay listener moves, and `MinimalDht.bootstrap` replaces its bootstrap set, forgetting a node switched off — also when a peer's find-value answer names it — until it is switched on again.
