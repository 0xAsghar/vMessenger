...
>
> **There is no migration path and none is planned.** Uninstall the 0.x app, then install the current release.
...
> had paired with must re-pair with you once you both have 1.x or later.
...
- Identity is an on-device Ed25519 key pair. The private key never leaves the device, except inside a passphrase-encrypted backup the user chooses to export.
...
5. Peers run the v2 handshake — three steps, the last two signed over the transcript, three X25519 DHs — and exchange ChaCha20-Poly1305 frames over a symmetric ratchet.
...
Implemented (the 1.0 core was verified on two emulators, [docs/Testing.md](docs/Testing.md) §4; New node against throwaway servers, §2.1–2.2):
...
- **Messaging** — 1:1 end-to-end encrypted chat with replies, edit and delete for everyone, delivery and read receipts (batched), a persistent encrypted outbox with backoff and a 24-hour retry window; a message for an offline contact can also be sealed to them and left for up to 24 hours with another of your contacts (store-and-forward, on by default — Security L3, L15). Self-destructing messages (a per-conversation timer: 1 hour, 24 hours, 7 days or a chosen date and time), multi-image albums, and sharing into a conversation from other apps' share sheets.
- **Groups** — up to 100 members, client-side fan-out over the existing pairwise sessions, with creator-authoritative versioned membership, gap detection and snapshot recovery, system lines for membership changes, and a closed group that keeps its history. The creator can name admins and switch on review of edited and deleted messages, which lets the creator and admins read on their own device what a message said before it was edited or withdrawn (off by default, never in a 1:1 chat, kept at most 90 days; switching it is recorded in the creator's copy of the group's history — [docs/Security.md](docs/Security.md) §14, L19). A group message's single tick is aggregated from per-recipient state; a long press opens a per-member delivered/read sheet.
- **Voice calls** — one-to-one, end-to-end encrypted with a per-call key, Opus audio over a direct connection or a relay circuit, reconnecting on its own after a network change; started from a chat, a contact's page or the contacts list.
...
- **Backup** — passphrase-protected backup bundle (Argon2id13 + XChaCha20-Poly1305) of the identity, contacts, location grants, user-added nodes and, optionally, conversations with their messages (attachment files are not included).
...
Not implemented: video calls, group calls, Bluetooth / Wi-Fi Direct / mesh transports, geofencing, location analytics, SOS mode, a plugin system.
...
The cryptographic and metadata gaps are enumerated as **L1–L22 in [docs/Security.md](docs/Security.md) §10**, with the file and flag behind each one. That table is the authoritative list and is not duplicated here. The four that most change what a user should expect:
...
- **Contact keys are trust-on-first-use**, and nothing forces a safety-number comparison: a contact whose key changes is unreachable until the new key is accepted on the contact's page, which is one tap whether or not anyone compared (L4, L5).
- **A group's membership is whatever its creator says it is.** Admins can be named, but there is no group key, no creator transfer and no member-side veto, and the fan-out pattern itself tells a relay which peers form a group (L12, L13).

One more from that table, about nodes rather than messages:

- **The operator trust anchor is a placeholder.** `NetworkConfig.OPERATOR_ED25519_PUBLIC_KEY_HEX` is 64 zeros, so no `SignedNodeRecord` can ever be marked `OFFICIAL`. No release so far has set it; the operator tier stays dead code until it is set (L6).
...
vMessenger/
  ...
  core/                <- common, crypto, proto, database, datastore, location, map, notifications, designsystem,
                          audio (call audio, Opus), ssh (SSH client), nodesetup (the New node engine)
  data/                <- repository implementations, network coordinators, attachment + wipe + backup, calls,
                          New node controller
  ...
  scripts/             <- setup-node.sh, provision-test/ (Docker harness), emulator-connect.sh, p2p-terminal-check.sh,
                          sign-node-record/ (the node-record signing contract; no signing tool in the repository)
...
Requirements: JDK 17 to run Gradle, which provisions the JDK 21 toolchain it compiles with (CI installs Temurin 17 and 21; the node's tests run on 17), Android SDK platform 35 (the Android Gradle plugin fetches the build tools it needs), and a `local.properties` with `sdk.dir`.
...
`unitTests` is the aggregate task — a plain `testDebugUnitTest` skips the JVM modules `:core:common`, `:domain`, `:core:ssh`, `:core:nodesetup` and `:node`. See [docs/Testing.md](docs/Testing.md).
...
```bash
git tag v2.0.1        # must equal versionName in gradle/version.properties
git push origin v2.0.1
```

The version is a single source of truth: `gradle/version.properties` sets it for the app and the
node tarball, and cutting a release means bumping both fields there and moving the `## [Unreleased]`
section of [CHANGELOG.md](CHANGELOG.md) under a dated `## [x.y.z]` heading.
`NetworkConfig.OPERATOR_ED25519_PUBLIC_KEY_HEX` is still the all-zero placeholder (see
...
Quick start by hand on a fresh Ubuntu/Debian host, from a checkout of this repository (without
`--dist-tar vmessenger-node-<version>.tar.gz`, the script downloads the node from the latest GitHub release):
